...
public:
		...
		: Layer("Example"), m_Camera(-1.6f, 1.6f, -0.9f, 0.9f), m_CameraPosition(0.0f)
	{
		...
		m_Shader.reset(new PolyEngien::Shader(vertexSrc, fragmentSrc));
		...
		m_BlueShader.reset(new PolyEngien::Shader(blueShaderVertexSrc, blueShaderFragmentSrc));
	}
	...
	void OnUpdate(PolyEngien::Timestep ts) override
	{
		...
		for (int y = 0; y < 20; y++)
		{
			for (int x = 0; x < 20; x++)
			{
				...
				PolyEngien::Renderer::Submit(m_BlueShader, m_SquareVA, transform);
			}
		}
...
private:
	...
	std::shared_ptr<PolyEngien::Shader> m_BlueShader;
